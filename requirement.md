# 音楽プレーヤー「UX Music」機能仕様書 (v0.1.7beta)
...
- **[最優先] 画面遷移ロジックの不具合による描画異常**:
  - **根本原因**: 画面遷移の堅牢性を高めるために導入した「表示領域の破壊と再構築」方式と、各画面の描画処理の連携が不完全な状態にある。
  - **現象**: アプリケーション起動後、「曲」ビューは正常に表示されるが、サイドバーから「アルバム」などの他のビューに切り替えると、画面が真っ白（または黒）になり、コンテンツが一切描画されない。デバッグツールで確認すると、`<main>`タグの中身が空のままになっている。
  - **分析**: 画面を切り替える際に、古い画面の要素（仮想スクローラーを含む）を破棄する処理は成功しているが、その後に**新しい画面の要素を構築・描画する処理が正しく呼び出されていない、または実行に失敗している**可能性が高い。
