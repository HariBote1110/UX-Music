# 音楽プレーヤー「UX Music」機能仕様書 (v0.1.5暫定)
...
- **[改善済み] UIパーツの視認性向上**:
    ...
    - **[改善済み] 右サイドバーのタブUI**: アクティブなタブを示す下線がタブ幅全体に表示されるよう修正し、デザイン性を向上。
    - **[改善済み] スクロールバーのデザイン**: アプリケーション全体のダークなトーンに合わせ、スクロールバーをカスタマイズ。
...
- **[UI/アニメーション] テキストのスクロールアニメーション**:
  - **現象**: マウスオーバー時に長いテキストをスクロールさせるアニメーションが意図通りに動作しない。JavaScriptによる「はみ出し判定」がDOMの描画完了前に実行されてしまい、全てのテキストが「はみ出していない」と誤判定されている。
  - **原因推測**: `requestAnimationFrame`を使用しても、コンポーネントの描画タイミングによっては幅の計算が`0px`となってしまう。イベントリスナーの登録と判定の実行タイミングを、より確実な方法（イベント委任など）に見直す必要がある。
...
- **テキストのフェードアウト効果**:
  - **内容**: 画面幅に収まらない長いテキストを「...」で省略するのではなく、テキストの端をフェードアウトさせて表示する。（実装済み、スクロールアニメーションとの連携を要調整）
