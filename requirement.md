# 音楽プレーヤー「UX Music」機能仕様書 (v0.1.4暫定)
...
- **[UI] 右サイドバーのタブ下線スタイル**:
  - **現象**: 「歌詞」と「再生キュー」を切り替えるタブで、アクティブ状態を示す下線が意図したデザイン（テキストの真下に短い線）で表示されない。
  - **原因推測**: 親要素の`border-bottom`と、アクティブなタブに適用している疑似要素`::after`のスタイルの競合、または優先順位の問題。複数回のCSS修正を試みたが、完全な解決には至っていない。
