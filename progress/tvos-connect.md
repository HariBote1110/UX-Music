...
## 追記（2026-08-12）: 受け口の認証トークンをホストのペアリングトークンから分離

上記「認証: 対称共有シークレット」節に記録した設計（TVの受け口がホストとの
ペアリングトークンをそのままBearer認証とmDNS TXTの両方に使う）を**セキュリティ
上の欠陥として撤回**し、TVの受け口専用の独立トークンに切り替えた。

- **問題**: `deviceAuthTokens`のペアリングトークンはHostの`/v1/remote/*`
  ライブラリAPI全体（曲一覧・ダウンロード等）への資格情報であり、TVの受け口だけが
  制御する「TV自身の再生」よりはるかに強い権限を持つ。このトークンをmDNS TXTに
  平文で載せることは、当初「TVは自分の再生しか制御できないため被害範囲が
  限定的」と評価した想定を裏切っていた——実際に漏れるのはTVの受け口用の
  権限ではなく、Host本体の権限だったため。LAN上の誰でもこのTXTレコードを
  読めばHostライブラリ全体にアクセスできてしまう状態だった。
- **修正**: `TVControlTokenStore`を新設し、TVの受け口が起動時に自分専用の
  ランダムなコントロールトークン（32バイト、hex 64文字）を生成・
  `UserDefaults`に永続化するようにした（`TVServerConfigStore`とは別キー
  `tv_control_token`）。`TVRemoteControlServer`はこのコントロールトークンで
  のみ認証し、mDNS TXTにもこれだけを載せる。ホストのペアリングトークン
  （`ServerConfig.token`）はTVの受け口のコード経路から完全に排除した。
- **リスク評価の再確認**: コントロールトークンが漏れた場合の被害範囲は
  「このTVの再生操作」のみに限定される。これは当初想定していた被害範囲と
  一致するため、「既存のオープンなペアリングエンドポイントと同程度の
  許容リスク」という評価はコントロールトークンについては引き続き妥当と判断した。
- **iOS側への影響なし**: `TVRemoteTarget.make`はTXTの`token`フィールドを
  そのまま読むだけで、値がコントロールトークンかホストトークンかを
  区別するロジックを持たない。フィールド名・構造は変更していないため、
  iOS側のピッカーUIはコード変更不要。
- **テスト**: `TVControlTokenStoreTests`（生成・永続化・別ストア間での
  非重複を確認）と、`TVRemoteCommandHandlerTests`に追加した
  `testHostPairingTokenDoesNotAuthorizeAgainstIndependentControlToken`
  （ホストトークンを提示しても、受け口の期待値がコントロールトークンなら
  認証を通らないことを確認）でこの分離を担保している。
