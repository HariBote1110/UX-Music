# 音楽プレーヤー「UX Music」機能仕様書 (v0.1.9 Beta 1a)

これは以前に使用していたファイルです。今はその他docディレクトリにあるマークダウンへと移行しています。

## 概要
ローカル・オンラインの音源を統合的に管理・再生できるデスクトップ音楽プレーヤー。Electronフレームワークを基盤とし、音源のインポート、再生、管理に関する多岐にわたる機能を提供する。継続的なデバッグと機能改善を経て、安定性とパフォーマンスが向上している。

## 技術スタック
- **フレームワーク:** Electron
- **言語:** JavaScript (Node.js), HTML5, CSS3
- **主要ライブラリ:**
  - `discord-rpc` (Discord Rich Presence連携)
  - `sharp` (アートワークのサムネイル生成・画像処理)
  - `@distube/ytdl-core` (YouTubeのメタデータ取得および動画ダウンロード)
  - `music-metadata` (音声ファイルのメタデータ解析)
  - `fluent-ffmpeg`, `ffmpeg-static` (音声ファイルのラウドネス値・BPM・雰囲気解析・ノーマライズ処理)
  - `music-tempo`, `wav-decoder`, `tmp` (BPM解析)
  - `electron-builder` (アプリケーションのビルド・パッケージング)

## 対応OS
- **主要開発・動作確認環境:** macOS (Windowsは公式には対応しない)

> [!WARNING]
> **ハイパワーマシン向け**
> このアプリケーションは、楽曲のインポートやノーマライズ処理時に音声解析を行うため、CPUリソースを消費する場合があります。設定から「パフォーマンスモード」を選択することで、PCのリソースを最大限活用し、処理を高速化できます。

---

## 主要機能

### [新規] 音量ノーマライズツール (UX Normalize)
ライブラリとは独立して、指定した音声ファイルの音量を一括で均一化するツールを内蔵。
- **ファイル読み込み**:
    - ファイル/フォルダ単位での追加、およびドラッグ＆ドロップに対応。
    - UX Musicのライブラリ全体を読み込み、解析済みのラウドネス値を活用することが可能。
- **高機能な解析と適用**:
    - **LUFSベース**: 業界標準のLUFSラウドネス単位に基づき、ターゲット音量をスライダーで直感的に設定可能。
    - **バックグラウンド処理**: 解析とノーマライズ（ファイル書き換え）処理は全てWorkerスレッドで実行され、UIの応答性を維持。
    - **柔軟なファイル選択**: チェックボックスで処理対象のファイルを個別に選択・除外できる。
- **安全なファイル出力**:
    - **保存先の選択**: 元のファイルを上書きするか、指定した別のフォルダに保存するかを選択可能。
    - **バックアップ機能**: 上書き保存時に、元ファイルのバックアップ(`.bak`)を自動で作成するオプションを提供。
    - **音質劣化への配慮**: MP3などの非可逆フォーマットを処理する際、再エンコードによる音質劣化の可能性があることをユーザーに警告し、同意を得てから実行する。

### 起動とパフォーマンス
- **[改善済み] 起動プロセスの最適化**:
  - **モジュールの遅延読み込み**: YouTube関連など、アプリ起動時に不要な機能のモジュール読み込みを遅延させ、メインプロセスの起動を高速化。
- **[改善済み] インポート処理の高速化と安定化**:
    - **バックグラウンド処理**: ラウドネス、BPM、Energyの解析処理を**Web Worker**に完全分離。これにより、重いインポート中でもUIが「応答なし」になる問題を根本的に解決。
    - **パフォーマンスモード**: PCの搭載メモリ量に応じて並列処理数を動的に増減させるインテリジェントなモードを実装。これにより、ハイスペックPCではその性能を最大限に活かした超高速インポートが可能に。
    - **軽量な音量解析**: 従来の`loudnorm`から高速な`volumedetect`および`astats`フィルターに切り替えたことで、CPU負荷と発熱を大幅に削減し、インポート時間を劇的に短縮（例: 10分→30秒）。
- **[改善済み] 待機中のリソース消費削減**:
    - **アイドル状態の検知**: 5分間ユーザーの操作がない場合に、CSSアニメーションなどを停止する「スリープモード」を実装。これにより、アプリを長時間起動したままでもCPUリソースをほとんど消費しない。
    - **ビジュアライザーのエコモード**: 再生中の曲が画面外にスクロールされると、ビジュアライザーの描画を完全に停止し、CPU負荷を大幅に軽減。
- **[新規] Light Flightモード (究極の軽量化)**:
    - **内容**: アプリケーションの動作を極限まで軽量化するためのモード。サイドバーからワンクリックで切り替え可能。
    - **機能**:
        - ジャケット画像などのサムネイルを一切描画せず、メモリ使用量を大幅に削減 (デバッグツール非表示時で**約100MB**)。
        - ビジュアライザーをCPU負荷ゼロの超軽量な**アニメーションSVG**に切り替え。
        - 描画フレームレートを**30FPS**に制限。

### ライブラリ管理
- **[改善済み] 堅牢なデータ管理**:
    - **ユニークID**: 各楽曲にユニークなID (UUID) を付与。これにより、ファイルパスの変更に影響されず、再生履歴などの関連データを正確に管理する。
- **[改善済み] YouTube機能のデフォルト無効化**:
  - **権利関係への配慮**: YouTube関連機能（リンク追加、プレイリストインポート）はデフォルトで非表示。
  - **オプトイン方式**: 開発者コンソールのデバッグコマンド `uxDebug.enableYouTubeFeatures()` を実行し、免責事項に同意した場合にのみ機能が有効化される。
- **[改善済み] 楽曲分析機能**:
    - **雰囲気分析(Energy)**: FFmpegの`astats`フィルターを利用し、曲のダイナミックレンジ（音量の起伏）を`Energy`として数値化。これにより、「For You」プレイリストの精度が向上。
    - **BPM解析**:
        - **[修正済み]** インポート時にBPMが欠落する問題を修正。
        - **[修正済み]** `fluent-ffmpeg`がハングアップする問題を、一時ファイルを利用する安定した方式に変更して解決。
        - **[改善済み]** BPMが2倍で検出される「倍テン」問題を、180BPMを超える場合に半減させるロジックを追加して補正。
    - **ジャンル取得**: インポート時に楽曲のメタデータからジャンル情報を取得し、ライブラリに保存。

### 再生機能
- **[新規] イコライザー機能 (EQ)**:
  - **Web Audio APIベース**: `BiquadFilterNode`を使用した高音質なイコライザーを実装。
  - **デュアルUI**:
    - **シンプルモード**: サイドバーに「Bass」「Mid」「Treble」の3バンドEQを配置し、直感的な調整が可能。
    - **グラフィックモード**: 設定画面に、20Hz〜20kHzをカバーする10バンドのグラフィックEQを実装。マウスドラッグでEQカーブを直接描画できる折れ線グラフ形式のUIを採用。
  - **プリセット**: 「Rock」「Pop」「Jazz」など複数のプリセットを搭載し、ワンクリックで設定を切り替え可能。詳細設定画面からもプリセットを選択できる。
  - **状態保存**: ON/OFFの状態や各バンドのゲイン値は自動で保存され、アプリ再起動後も復元される。
- **オーディオノーマライザー**: 楽曲ごとにラウドネス値（LUFS）を解析し、再生音量を自動で均一化。
- **多彩な再生モード**: 通常再生、全曲リピート、1曲リピート、シャッフル再生に対応。
- **高品質なUI**:
    - `requestAnimationFrame`により、滑らかに動作するシークバー。
    - OSのメディアキー（再生/一時停止、曲送り/戻し）からの操作に対応。
- **再生デバイスの切り替え**: アプリ内のポップアップから、オーディオの出力先を動的に変更可能。

### For You 機能 (自動プレイリスト生成)
- **[新規] パーソナルプレイリスト**:
    - **再生日時の記録**: 曲の再生履歴（いつ聴いたか）を記録する仕組みを追加。
    - **再生傾向の分析**: 記録されたデータを基に、以下のプレイリストを自動生成。
        - **最近のお気に入り**: 直近2週間でよく聴いた曲。
        - **ちょっと前のお気に入り**: 2週間〜3ヶ月前によく聴いていた、最近はあまり聴いていない曲。
        - **変わらない愛曲**: 総再生回数が多く、長期的に愛聴されている曲。
- **[改善済み] シチュエーションプレイリスト**:
    - **動的生成**: 現在の**時刻**（朝、昼、夕方、深夜）や**季節**（夏、冬）に応じて、表示されるプレイリストが自動で変化。
    - **高度なルール設定**: BPM、ジャンル、Energy、タイトルキーワードを組み合わせ、さらに特定のキーワードを除外（減点）するルールを導入し、プレイリストの精度を大幅に向上。（例: 「爽やかな朝に」プレイリストから「夜」という単語が入った曲を除外）
    - **[新規]** 設定画面からイースターエッグ（深夜4時台の特別プレイリスト）の表示/非表示を切り替え可能に。

### UI・UX
- **[新規] アルバムアートのグループ表示**:
  - **概要**: 設定から有効にできる、リスト表示用の新しい表示モード。
  - **機能**:
    - 同じアルバムの楽曲がリストに連続して並んでいる場合、最初の1曲にのみジャケット画像を表示する。
    - 2曲目以降はジャケット画像を非表示にし、代わりにツリー状の罫線（`│` や `└`）を表示することで、視覚的にアルバムのまとまりを示す。
    - これにより、リストの縦方向のスペースを節約し、視認性を向上させる。
- **[新規] 複数選択とクリップボード操作**:
    - `Cmd/Ctrl + クリック` での複数選択、`Cmd/Ctrl + A` での全選択に対応。
    - `Escape`キーで全ての選択を解除。
    - 選択中の曲は青くハイライトされ、視覚的に分かりやすい。
    - `Cmd/Ctrl + C`で選択中の曲をコピーし、`Cmd/Ctrl + V`で表示中のプレイリストに一括で追加可能。
- **[改善済み] スクロールパフォーマンス**:
    - **`transform`の活用**: リスト項目の位置決定に`transform`プロパティを使用することで、スクロール時のCPU負荷を軽減し、GPUアクセラレーションを促進。
    - **[改善済み] 仮想スクロールの適用範囲拡大**: アルバム詳細やプレイリスト詳細画面にも仮想スクロールを導入し、数百曲以上のリストでも軽快な動作を実現。
- **[改善済み] UIレイアウト**:
  - **アクションメニュー**: サイドバー左下のアクションボタン群を「ライブラリを管理」ポップアップメニューに集約し、ウィンドウサイズが小さくても操作性を損なわないデザインに改善。
  - **アプリアイコン**: 背景を透過させ、macOSのDockなどでより自然に表示されるように修正。
- **[改善済み] オーディオデバイス管理**:
  - **デバイスの非表示**: 設定画面から、不要なオーディオデバイス（例: VoiceMeeter）を再生先リストから非表示にできる機能を追加。
  - **右クリックメニュー**: 再生デバイスリスト内の項目を右クリックし、「このデバイスを非表示にする」を選択することで、素早くリストを整理できる。
- **[新規] ハイレゾ音源の特別表示**:
    - インポート時にハイレゾ音源（48kHz/16bitを超える）を自動で判別。
    - 対象の楽曲には、リスト上で特別な**HR**アイコンが表示され、一目で識別可能に。

### 外部連携 (Integrations)
- **[新規] Discord Rich Presence**:
  - アプリで再生中の楽曲（曲名、アーティスト名）を、Discordのステータスにリアルタイムで表示。
  - SpotifyやVS Codeのように、友人に聴いている曲を共有できる。

---

## 今後の課題・将来的な拡張案

- **メタデータ編集機能の実装**:
  - **内容**: アプリ上から曲名、アーティスト名、アルバム名、ジャケット画像などを直接編集し、ファイルに書き込む機能。
- **MP4ファイルのサムネイル自動設定**:
  - **内容**: ユーザーがインポートした映像付きMP4ファイルにアートワークが設定されていない場合、動画のフレームから自動でサムネイルを生成・設定する。
- **検索・フィルタリング機能の実装**:
  - **内容**: 曲、アルバム、アーティストなどをキーワードで絞り込める検索バーを実装する。
- **歌詞の自動取得機能**:
  - **内容**: ローカルに歌詞ファイルがない場合、オンラインサービスから歌詞を自動で検索・取得する機能。
- **UX Musicアプリ間の同期機能 (優先度: 低)**:
  - **内容**: WebSocketやHTTP通信を利用し、異なるPCで利用しているUX Musicアプリ間で再生回数などのデータを同期する。