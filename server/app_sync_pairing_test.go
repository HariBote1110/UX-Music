...
func TestSyncPairingStartAndConfirmIssuesToken(t *testing.T) {
	newTempSyncStore(t)
	startReq := httptest.NewRequest(http.MethodPost, "/v1/pairing/start", bytes.NewReader([]byte(`{"deviceId":"macbook-air"}`)))
	...
	confirmReq := httptest.NewRequest(http.MethodPost, "/v1/pairing/confirm", bytes.NewReader(confirmPayload))
...
func TestSyncPairingConfirmStoresKnownPeerFromInboundRequest(t *testing.T) {
	newTempSyncStore(t)
	startReq := httptest.NewRequest(http.MethodPost, "/v1/pairing/start", bytes.NewReader([]byte(`{
...
func TestDeviceAuthMiddlewareRequiresTokenForLibraryEvents(t *testing.T) {
	newTempSyncStore(t)
	token := ensureDeviceAuthToken("macbook-air")
	...
	handler.ServeHTTP(missing, httptest.NewRequest(http.MethodPost, "/v1/sync/library/events", nil))
	if missing.Code != http.StatusUnauthorized {
		t.Fatalf("expected unauthorised without token, got %d", missing.Code)
	}

	withToken := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/v1/sync/library/events", nil)
...
func TestDeviceAuthMiddlewareSharesOneTokenAcrossSyncAndRemoteRoutes(t *testing.T) {
	newTempSyncStore(t)
	token := ensureDeviceAuthToken("macbook-air")
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusAccepted)
	...
	syncReq := httptest.NewRequest(http.MethodPost, "/v1/sync/library/events", nil)
	syncReq.Header.Set("Authorization", "Bearer "+token)
	syncRec := httptest.NewRecorder()
	handler.ServeHTTP(syncRec, syncReq)
	if syncRec.Code != http.StatusAccepted {
		t.Fatalf("expected device token to pass sync route, got %d", syncRec.Code)
	}

	remoteReq := httptest.NewRequest(http.MethodGet, "/v1/remote/songs", nil)
	remoteReq.Header.Set("Authorization", "Bearer "+token)
	remoteRec := httptest.NewRecorder()
	handler.ServeHTTP(remoteRec, remoteReq)
	if remoteRec.Code != http.StatusAccepted {
		t.Fatalf("expected the same device token to pass the remote route, got %d", remoteRec.Code)
	}
}

// corruptSyncToken は文字数を保ったまま末尾 1 文字だけ差し替えた別トークンを返す。
// deviceTokenIsValid は長さが違うと ConstantTimeCompare まで到達しないため、
...
func TestDeviceAuthMiddlewareRejectsWrongSyncToken(t *testing.T) {
	newTempSyncStore(t)
	valid := ensureDeviceAuthToken("macbook-air")
...
func TestDeviceAuthMiddlewareRejectsTokenOfUnpairedSyncDevice(t *testing.T) {
	newTempSyncStore(t)
	// dev_paired だけをペアリング済みにし、別デバイス用に発行された形式的に正しい
	// トークンを提示する。ストアの tokens マップに載っていないので拒否されるはず。
	paired := ensureDeviceAuthToken("dev_paired")
...
func TestSyncPairingConfirmRejectsInvalidRequests(t *testing.T) {
	t.Run("expired session", func(t *testing.T) {
		...
		rec := httptest.NewRecorder()
		syncPairingConfirmHandler(rec, httptest.NewRequest(http.MethodPost, "/v1/pairing/confirm", bytes.NewReader(payload)))
	...
	t.Run("consumed session id", func(t *testing.T) {
		newTempSyncStore(t)
		startRec := httptest.NewRecorder()
		syncPairingStartHandler(startRec, httptest.NewRequest(http.MethodPost, "/v1/pairing/start", bytes.NewReader([]byte(`{"deviceId":"dev_air"}`))))
		if startRec.Code != http.StatusOK {
			t.Fatalf("unexpected start status %d: %s", startRec.Code, startRec.Body.String())
		}
		var started syncPairingStartResponse
		...
		syncPairingConfirmHandler(firstRec, httptest.NewRequest(http.MethodPost, "/v1/pairing/confirm", bytes.NewReader(payload)))
		...
		syncPairingConfirmHandler(secondRec, httptest.NewRequest(http.MethodPost, "/v1/pairing/confirm", bytes.NewReader(payload)))
