...
/// static imagesets, and for the iPhone app's static SF Symbols (`"shuffle"`/`"repeat"`/
/// `"repeat.1"`). The desktop app's signature move on these buttons is that, on tap, the stroke
/// visibly slides along its own path (`runShuffleAnimation`/`runLoopAnimation` in
/// `src/renderer/js/ui/player-ui.ts`) — a static raster/SVG/SF Symbol asset cannot express that, so
/// the icons are drawn as `Path`s here instead and animated via `ModeIconAnimator`. The underlying
...
///
/// Shared across the iOS app and Watch targets (see `WatchModeIconViews.swift` for the Watch's
/// thin wrappers) so both platforms reproduce the exact same desktop-derived geometry and timing.
...
/// with `scale: 1` — the outer scale applies to that offset along with everything else. Internal
/// (not `private`) because both the Watch wrappers (`WatchModeIconViews.swift`) and the iOS
/// wrappers below construct it directly.
struct AnimatedTwoStrokeIcon: View {
    let topPath: Path
    let bottomPath: Path
    let topHead: Path
    let bottomHead: Path
...
/// Shuffle icon *content only* — no tap frame, no background/pill styling — so each platform's
/// button chrome (the Watch's plain tap frame, the iPhone's accent pill + dot in
/// `NowPlayingTransportSection`) can wrap it as needed. Owns its own `ModeIconAnimator` and
/// triggers the slide animation whenever `isActive` changes, i.e. on every tap of the shuffle
/// button (`toggleShuffle()` flips `isActive` unconditionally).
struct ShuffleModeIcon: View {
    let isActive: Bool
    var iconSize: CGFloat = 20
    var activeTint: Color = .blue
    var inactiveTint: Color = .white.opacity(0.6)
...
/// Repeat icon *content only* — see `ShuffleModeIcon`'s doc comment for why chrome is left to the
/// caller. Draws the "1" badge for `.one` when `showsOneBadge` is set (both platforms want it, but
/// its size/positioning differs enough per platform that callers pass their own flag rather than
/// this view hardcoding one look). Triggers the slide animation whenever `repeatMode` changes, i.e.
/// on every tap of the repeat button (`cycleRepeatMode()` always advances to a different mode).
struct RepeatModeIcon: View {
    let repeatMode: PlaybackRepeatMode
    var iconSize: CGFloat = 20
    var activeTint: Color = .blue
    var inactiveTint: Color = .white.opacity(0.6)
    var showsOneBadge: Bool = true

    @StateObject private var animator = ModeIconAnimator(staggerSeconds: 0)

    private var tint: Color { repeatMode == .off ? inactiveTint : activeTint }
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            if showsOneBadge && repeatMode == .one {
                Text("1")
