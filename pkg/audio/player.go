...
func (p *Player) Play(filePath string, gainLinear float64) error {
	...
	case ".flac":
		...
		if err != nil {
			// ネイティブFLACデコーダーがオープンに失敗した場合のみFFmpegにフォールバック
			// （ミッドストリームのフォールバックは行わない: デコーダーはpanicせず
			// エラーを返すため、破損ストリームを途中で切り替えるより明確なエラーの方が良い）
...
func resolveCommandPath(name string) (string, error) {
	...
	for _, candidate := range buildCommandFallbackCandidates(name) {
		...
		fmt.Printf("[Audio] command fallback resolved: %s -> %s\n", name, candidate)
