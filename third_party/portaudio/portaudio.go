...
extern void* paStreamUserData(uintptr_t id);
*/
import "C"
...
func OpenStream(p StreamParameters, args ...interface{}) (*Stream, error) {
	...
	paErr := C.Pa_OpenStream(&s.paStream, s.inParams, s.outParams, C.double(p.SampleRate), C.ulong(p.FramesPerBuffer), C.PaStreamFlags(p.Flags), cb, C.paStreamUserData(C.uintptr_t(s.id)))
	if paErr != C.paNoError {
		delStream(s)
...
func setChannels(s *reflect.SliceHeader, p uintptr, frames int) {
	for i := 0; i < s.Len; i++ {
		buf := unsafe.Add(unsafe.Pointer(s.Data), unsafe.Sizeof(reflect.SliceHeader{})*uintptr(i))
		setSlice((*reflect.SliceHeader)(buf), *(*uintptr)(unsafe.Add(nil, p)), frames)
