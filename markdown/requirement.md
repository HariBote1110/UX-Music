# 音楽プレーヤー「UX Music」機能仕様書 (v0.1.9-Beta-22a)
...
- **転送進捗と圧縮転送**: UX Sync 専用設定画面は転送中のファイル名、件数、転送量、転送速度を表示する。`PushSyncLibraryAssetsWithOptions(baseURL, limit, { encodingMode: "mp3_320" })` により、FLAC などの重い音源を送信前に MP3 320kbps へ変換してから転送できる。
