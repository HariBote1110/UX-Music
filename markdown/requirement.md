# 音楽プレーヤー「UX Music」機能仕様書 (v0.1.9-Beta-35b)
...
- **子側pull取り込み**: `PullSyncLibraryAssets(baseURL, limit)` は保存済み `syncAuthTokens` を使って親へ接続し、受信音源を子側ユーザーデータ配下の `SyncLibrary` に保存し、`library.json` へ `syncSourceDeviceId` / `syncSourceTrackId` 付きで取り込む。受信した `syncPlayCount` は実保存パスの `playcounts` に反映し、既に取得済みの曲は音源を再取得せず再生回数だけ更新する。`settings.syncPreferredFormat="mp3_320"` の端末は対応 peer から MP3 320kbps を取得し、非対応 peer では原本取得へフォールバックする。
