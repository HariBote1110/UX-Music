...
- [ ] Wails プロジェクトが構成され、既存の `src/go` のロジックを呼び出せる `App` 構造体が定義されていること。
- [ ] `src/renderer` が Wails の `frontend` として機能するためのディレクトリ構成が完了していること。
- [ ] フロントエンドから Go のメソッドを呼び出すサンプルが動作すること。
